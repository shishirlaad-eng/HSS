 ...
 * itemsPerPage === 0 is the sentinel for "View All" (no pagination)
 */
...
export function Pagination({
  ...
  const isViewAll = itemsPerPage === 0;
  const startItem = isViewAll ? 1 : (currentPage - 1) * itemsPerPage + 1;
  const endItem   = isViewAll ? totalItems : Math.min(currentPage * itemsPerPage, totalItems);
  ...
  return (
    <div className="flex items-center justify-between border-t border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 px-4 py-3 mt-6">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Page navigation – hidden when viewing all */}
        {!isViewAll && (
          <div className="flex items-center gap-2">
            {/* Previous button */}
            <button
