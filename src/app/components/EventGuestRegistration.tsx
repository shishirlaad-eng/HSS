...
import {
  ArrowLeft, Clock, MapPin, Globe, Users as UsersIcon, CreditCard, ScrollText,
  ListChecks, Check,
} from 'lucide-react';
...
const DIETARY_OPTIONS = [
  'Coeliac', 'Gluten-free', 'Vegan', 'Lacto (allows dairy)', 'Paleo Diet',
  'Ketogenic (low carbohydrate, high fat)', 'Low GI (limits carbohydrate intake)',
  'FODMAP', 'No Onions or Garlic', 'Other',
];

const STATUS_LABEL: Record<string, string> = {
  draft: 'Draft', published: 'Published', active: 'In Progress',
  cancelled: 'Cancelled', completed: 'Completed',
};

function formatPriceRange(priceCategories?: { price: number }[], price?: number): string {
  if (priceCategories && priceCategories.length > 0) {
    const prices = priceCategories.map(c => c.price);
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-lg p-5 flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-col gap-1 text-sm text-neutral-700 dark:text-neutral-300 mb-3">
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-success-50 dark:bg-success-950/20 text-success-700 dark:text-success-400 border border-success-200 dark:border-success-800 font-medium">
                {STATUS_LABEL[event.status] ?? event.status}
              </span>
              <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 font-medium">
                {event.paymentType === 'paid' ? `Paid · ${formatPriceRange(event.priceCategories, event.price)}` : 'Free'}
              </span>
              {event.capacity && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 font-medium">
              ...
              <FormField>
                ...
                <div className="flex flex-wrap gap-2">
                  {DIETARY_OPTIONS.map(opt => {
                    const checked = formData.dietaryRequirements.includes(opt);
                    return (
                      <label key={opt} className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg border cursor-pointer ${checked ? 'border-primary-500 bg-primary-50 dark:bg-primary-950/30 text-primary-700 dark:text-primary-300' : 'border-neutral-300 dark:border-neutral-700 text-neutral-600 dark:text-neutral-400'}`}>
                        <input type="checkbox" checked={checked} onChange={e => {
                          const next = e.target.checked ? [...formData.dietaryRequirements, opt] : formData.dietaryRequirements.filter(o => o !== opt);
                          set('dietaryRequirements', next);
                        }} className="hidden" />
                        {opt}
                      </label>
                    );
